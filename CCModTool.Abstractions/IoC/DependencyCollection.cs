...
using CCModTool.Abstractions.IoC.Exceptions;

namespace CCModTool.Abstractions.IoC;

public delegate T DependencyFactoryDelegate<out T>() where T : class;

public delegate T DependencyFactoryBaseGenericLazyDelegate<out T>(Type type, IDependencyCollection services)
	where T : class;
...
public sealed class DependencyCollection : IDependencyCollection
{
	...
	private delegate T DependencyFactoryDelegateInternal<out T>(IReadOnlyDictionary<Type, object> services)
		where T : class;
	...
	/// </summary>
	private readonly ConcurrentDictionary<Type, object> _lazyServices = [];

	private readonly Dictionary<Type, Type> _resolveTypes = [];
	private readonly Queue<Type> _pendingResolves = [];
	private readonly Dictionary<Type, DependencyFactoryDelegateInternal<object>> _resolveFactories = [];

	private readonly ConcurrentDictionary<Type, DependencyFactoryBaseGenericLazyDelegate<object>>
		_baseGenericLazyFactories = [];
	...
	public DependencyCollection(IDependencyCollection parent) =>
		_parentCollection = parent;
	
	public IDependencyCollection FromParent(IDependencyCollection parent) =>
		new DependencyCollection(parent);

	public IEnumerable<Type> GetRegisteredTypes() =>
		_parentCollection is not null
	...
	public Type[] GetCachedInjectorTypes()
	{
		...
		return _injectorCache.Where(kv => kv.Value.Delegate is not null).Select(kv => kv.Key).ToArray();
	}
	...
	public bool TryResolveType(Type objectType, [MaybeNullWhen(false)] out object instance) =>
		TryResolveType(objectType, _services, out instance);

	public bool TryResolveType(Type objectType, FrozenDictionary<Type, object> services,
		[MaybeNullWhen(false)] out object instance) =>
		TryResolveType(objectType, (IReadOnlyDictionary<Type, object>)services, out instance);
	...
	private bool TryResolveType(
		...
		[MaybeNullWhen(false)] out object instance)
	{
		if (services.TryGetValue(objectType, out instance)) return true;
	...
	public void Register<TInterface, TImplementation>(bool overwrite = false) where TInterface : class
